"""
Script to run sensitivity analysis (based on stage 2 - see docstring there for more info).

This version just runs for a single algorithm.

The following sensitivity scenarios are in use:
A: Overcommitted
B: Undercommitted
C: Emergent
D: Rigid
...
We will run each of these scenarios for 10 repeat ABM simulations, using only the nonlinear objective function
for the following team allocation algorithms:
- Random, 20 + 100 workers
- Greedy, 20 + 100 workers
- GRASP, 20 + 100 workers
- Linear, 20 + 100 workers
- RLD2_100_10, 20 + 100 workers
- Ensemble, 20 + 100 workers
"""
import pickle
import shutil
import os
...
GLOBALS = {
    'RUN_ID': 0,
    'REP_ABM': 10,  # Number of repeat ABM simulations to run
...
results_dir = Path(
    f"/media/data/rusty_data/sensitivity_results/single_algorithm/scenario_{SCENARIO}"
